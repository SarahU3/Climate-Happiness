...
X2<-cbind(emissions, Use, concerned)#needs to be updated based on updatedvariable list
...
stargazer::stargazer(L5, L6, L7, L8, covariate.labels = labels,
...
Emissions_range<-1:17 #need to fix this range
...
drawn <- mvrnorm(n = 1000, mu = L6_coef, Sigma = L6_vcov) %>% 
            data.frame
head(drawn)[1:3, ]
...
drawn_sim <- dplyr::rename(drawn_sim, fitted_emissions = y)
...
central <- drawn_sim %>% group_by(fitted_emissions) %>%

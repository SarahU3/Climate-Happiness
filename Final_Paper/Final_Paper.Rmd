...
``` {r individual graphs, echo = FALSE, messages = FALSE, warnings = FALSE}
...
individMeans <- aggregate(data[, c("GrossIncome", "age", "fam", "emp")], by = list(Year), FUN = mean)
...
indiplot1 <- ggplot(individMeans, aes(x = Year, y = MeanAge)) + geom_line() +labs(y = "Average age")
indiplot2 <- ggplot(individMeans, aes(x = Year, y = MeanIncome)) + geom_line() +labs(y = "Average income")
indiplot3 <- ggplot(individMeans, aes(x = Year, y = AveFam)) + geom_line() +labs(y = "Percentage with a partner")

pushViewport(viewport(layout = grid.layout(3, 2, heights = unit(c(1, 4, 4), "null"))))
grid.text("Sample Averages Over Time", vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2))
print(indiplot1, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(indiplot2, vp = viewport(layout.pos.row = 2, layout.pos.col = 2))
print(indiplot3, vp = viewport(layout.pos.row = 3, layout.pos.col = 1:2))
...
plot2 <- ggplot(data = aggremeans, aes(x=WorkHours, y=satis)) + geom_point() +geom_smooth() + labs(x = "Work Hours per Week", y = "Life Satisfaction")
...
plot5 <- ggplot(data = aggremeans, aes(x=Emissions, y=satis)) + geom_point() +geom_smooth() + labs(x = "Emissions per Capita", y = "Life Satisfaction")
...
statebox1 <- ggplot(data, aes(x=State, y=Emissions), main = "Emissions per Capita by State", xlab = "State", ylab = "Emissions (annual tons of CO2 per capita)") +geom_boxplot(fill ="firebrick", colour="black") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + guides(fill=FALSE)
statebox2 <- ggplot(data, aes(x=State, y=CO2perSqKm), main = "Emissions per Square Kilometer by State", xlab = "State", ylab = "Emissions (annual tons of CO2 per sq km)") +geom_boxplot(fill ="firebrick", colour="black") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + guides(fill=FALSE)
multiplot(statebox1, statebox2, cols=1)
```

...
## Get error in running all code after this point: "Error in pdim.default(index[[1]], index[[2]]) :"

## This is because there are multiple "State" values for each year - resulting from multiple individual data points with the same home state - would need to aggregate state data before completing state-level regression

#Aggregate state data to use state means for each year based on individual data
aggstatedata <-aggregate(data, by=list(State,Year), FUN=mean, na.rm=TRUE)
...
#set data as panel data
pdatastate<-plm.data(aggstatedata, index=c("Group.1","Year"))
...
#pooled OLS estimator
L5<-plm(Y2~X2, data=aggstatedata, model="pooling")
summary(L5)

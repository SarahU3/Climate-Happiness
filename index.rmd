...
```{r, echo=FALSE}
## Here we animate the emissions by country from 1990
...
agg <- aggregate(cbind(Emissions) ~ State + Year, FUN=mean, data=data)

#Transform yearly data to numeric
transform(data, Year = is.numeric(Year))
...
New <- FindReplace(data=agg, Var = "State", replaceData = Replaces, from = "from", to = "to",
  exact = TRUE, vector = FALSE)

New[, c(2,3)] <- sapply(New[, c(2,3)], as.numeric)


#set mins and max, start/end dates
startYear <- min(na.omit(New)$Year)
endYear <- max(na.omit(New)$Year)
minEmissions <- floor(min(New$Emissions, na.rm=TRUE))
maxEmissions <- ceiling(max(New$Emissions, na.rm=TRUE))

myColourAxis <- paste("{values: [0, 5, 10, 15, 20, 30, 35], ",
                      "colors: [\'red\', \'#D0D1E6\', \'#A6BDDB\', \'#74A9CF\',",
                      " \'#3690C0\', \'#0570B0\', \'green\']}")

## Create Geo Charts for each year

geoChartDE <- list(displayMode="regions",
                   region="DE", 
                   resolution="provinces",
                   legend="{numberFormat:'#,###.00'}") 


gvisData <- by(New, list(year=New$Year), function(x){
  
	year <- x$year[1]	
	g <- gvisGeoChart(x,  "State", "Emissions",
		options=geoChartDE,
...
Animation.playAnimation = function() {
  ...
  'Emissions per person in <b>'
 + Animation.currentYear + '</b> (States with no data shown in white)';
...
<title>Emissions per person by State</title>

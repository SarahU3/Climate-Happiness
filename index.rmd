...
library(googleVis)
require(datasets)
states <- data.frame(State, Emissions)
...
geostates2<-gvisGeoChart(data, locationvar = "State", colorvar = "Emissions",
  hovervar = "Emissions", options = list(region="DE"), chartid)
```
...
library(plotly)
emissbox <- boxplot(Emissions~State,
  data=data,
...
plot_ly(data, x = Year, y = Emissions, type = 'line', group = State)
```
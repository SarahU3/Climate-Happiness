...
transform(data, Year = as.numeric((Year)))
...
startYear <- min(New$Year)
endYear <- max((New)$Year)
...
plot(
  gvisData <- by(New, list(year=New$Year), function(x){
	year <- x$year[1]	
	g <- gvisGeoChart(x, locationvar = "State", 
               colorvar = "Emissions",
               options=geoChartDE,
               chartid=paste("[", year - startYear +1, "]", sep=""))
	.data <- g$html$chart["jsData"]
...
Animation.playAnimation = function() {
 ...
 + Animation.currentYear + '</b> (Provinces with no data shown in white)';
